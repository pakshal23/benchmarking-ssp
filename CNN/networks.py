...
class ResUNet(nn.Module):
    ...
    def create_model(self, kernel_size, num_channels):
        ...
        block_3 = [nn.MaxPool1d(kernel_size=2)]
        block_3.extend(self.add_block_conv(in_channels=2*num_channels, out_channels=4*num_channels, kernel_size=kernel_size, stride=1,
                                           padding=padding, batchOn=True, ReluOn=True))
        block_3.extend(self.add_block_conv(in_channels=4*num_channels, out_channels=4*num_channels, kernel_size=kernel_size, stride=1,
                                           padding=padding, batchOn=True, ReluOn=True))
        block_3.extend(self.add_block_conv_transpose(in_channels=4*num_channels, out_channels=2*num_channels, kernel_size=kernel_size, stride=2,
                                                     padding=padding, output_padding=1, batchOn=True, ReluOn=True))
    ...
    def forward(self, y_batch):
        ...
        block_3_output = self.block_3(block_2_1_output)
        result = self.block_2_2(torch.cat((block_2_1_output, block_3_output), dim=1))
