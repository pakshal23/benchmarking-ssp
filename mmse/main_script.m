...
%% Settings
data_folder = '/Users/bohra/Desktop/nn-vs-mmse/Data/Stable/FS/';
experiment = 'fourier_samp';     % 'deconv_gaussian' or 'deconv_airy_disk' or 'fourier_samp'
sig_type = 'alpha-stable';       % 'student' or 'alpha-stable' or 'bernoulli-laplace'
data_label = 'valid';            % 'train' or 'valid' or 'test'

exp_param = 31;     % parameter for the chosen experiment (see generate_datasets.m for more details)
...
opt_params.name = 'fista';      % 'fista' or 'admm'
opt_params.maxiter = 1e5;
opt_params.relative_tol = 1e-6;
...
lambda_list = 10.^(-10 : 0.5 : 8);   % Range of lambda values to tune over (for l1, l2 and log estimators)
...
x_l2 = cell([num_signals, num_lambda]);
err_l2 = zeros([num_signals, num_lambda]);
opt_cost_l2 = zeros([num_signals, num_lambda]);
num_iter_l2 = zeros([num_signals, num_lambda]);
...
parfor j = 1:num_signals
    ...
    % l2-estimator
    [x_l2(j,:), err_l2(j,:), opt_cost_l2(j,:), num_iter_l2(j,:)] = tune_lambda(l2_estimator_fun, meas, sig, lambda_list, [], opt_params, x0val.Value);
...
parfor j = 1:num_signals
    ...
    % log-estimator
    [x_log(j,:), err_log(j,:), opt_cost_log(j,:), num_iter_log(j,:)] = tune_lambda(log_estimator_fun, meas, sig, lambda_list, [], curr_x0_log);
